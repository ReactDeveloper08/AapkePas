...
public class RNVoipNotificationHelper {
    ...
    public void sendNotification(ReadableMap json) {
        ...
        NotificationCompat.Action rejectCall = new NotificationCompat.Action.Builder(R.drawable.rjt_btn,
                getActionText("Decline", android.R.color.holo_red_light),
                getPendingIntent(notificationID, "callDismiss", json)).build();
        NotificationCompat.Action acceptCall = new NotificationCompat.Action.Builder(R.drawable.acpt_btn,
                getActionText("Answer", android.R.color.holo_green_light),
                getPendingIntent(notificationID, "callAnswer", json)).build();
    ...
    public void createCallNotificationChannel(NotificationManager manager, ReadableMap json) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel(callChannel, json.getString("channel_name"),
                    NotificationManager.IMPORTANCE_HIGH);
            channel.setDescription("Call Notifications");
            channel.setSound(sounduri,
                    new AudioAttributes.Builder().setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION)
                            .setUsage(AudioAttributes.USAGE_UNKNOWN).build());
            channel.setVibrationPattern(new long[] { 0, 1000, 500, 1000, 0, 1000, 500, 1000, 0, 1000, 500, 1000, 0,
                    1000, 500, 1000, 0, 1000, 500, 1000, 0, 1000, 500, 1000 });
    ...
    public void showMissCallNotification(String title, String body, String callerId) {
        ...
        PendingIntent contentIntent = PendingIntent.getActivity(context, missNotification, intent,
                PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public void createNotificationChannel(NotificationManager manager, Uri sounduri) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            @SuppressLint("WrongConstant")
            NotificationChannel channel = new NotificationChannel(notificationChannel, "missed call",
                    NotificationManager.IMPORTANCE_HIGH);
            channel.setDescription("Call Notifications");
            channel.setSound(sounduri,
                    new AudioAttributes.Builder().setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION)
