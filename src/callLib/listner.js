...
const didReceiveStartCallAction = handler => {
  ...
  return eventEmitter.addListener(RNVoipCallDidReceiveStartCallAction, data =>
    handler(data),
  );
};
...
const didDisplayIncomingCall = handler =>
  eventEmitter.addListener(RNVoipCallDidDisplayIncomingCall, data =>
    handler(data),
  );

const didPerformSetMutedCallAction = handler =>
  eventEmitter.addListener(RNVoipCallDidPerformSetMutedCallAction, data =>
    handler(data),
  );
...
const didPerformDTMFAction = handler =>
  eventEmitter.addListener(RNVoipCallDidPerformDTMFAction, data =>
    handler(data),
  );
